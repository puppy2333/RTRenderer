...
public:
     ...
     */
    virtual float sample(Interaction& interact, std::vector<float>& cdf) = 0;
...
public:
    ...
    virtual float sample(Interaction& interact, std::vector<float>& cdf) override;
...
/**
 * Ideal SubsurfaceScatter BSSRDFs
 */

class IdealSubsurfaceScatter : public BRDF
{
protected:
    float ior;
    Eigen::Vector3f reflectivity;
    float scatter_dist;
    float calFresnel(Eigen::Vector3f normal, Eigen::Vector3f w);
    void sampleBurleyDiffusionProfile(float u, float rcpS, float& r, float& rcpPdf);

public:
    IdealSubsurfaceScatter(Eigen::Vector3f reflec, float idx_refract, float scatter_d);
    virtual Eigen::Vector3f eval(const Interaction& interact) override;
    virtual float samplePdf(const Interaction& interact) override;
    virtual float sample(Interaction& interact, std::vector<float>& cdf) override;
    virtual bool isDelta() const override;
